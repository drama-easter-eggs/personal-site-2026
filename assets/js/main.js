...
(function () {
  ...
  var REVEAL = [
    '.eyebrow',
    '.col > *:where(:not(.threads, .cases, .counters, .cv, .teach, .talks, .cards, .lenses, .fit, .teaching))',
    '.threads > *',
    '.cases > *',
    '.counters__item',
    '.cv > *',
  ...
  /* ---- 3. Industry rail: native scrolling + gentle automatic movement ---- */
  var rail = document.querySelector('[data-industry-rail]');
  if (rail) {
    var viewport = rail.querySelector('.industries__viewport');
    var track = rail.querySelector('.industries__track');
    var group = rail.querySelector('.industries__group');
    var motionPreference = window.matchMedia('(prefers-reduced-motion: reduce)');
    // Original list stays in the middle, with decorative copies on both sides.
    [true, false].forEach(function (before) {
      var copy = group.cloneNode(true);
      copy.removeAttribute('aria-labelledby');
      copy.setAttribute('aria-hidden', 'true');
      if (before) track.insertBefore(copy, group);
      else track.appendChild(copy);
    });
    rail.classList.add('is-ready');
    var span = 0, position = 0, lastWritten = 0, lastFrame = 0;
    var hovering = false, touching = false, nativeTouchActive = false, dragging = false, inView = true;
    var resumeAt = 0, dragX = 0, dragScroll = 0;
    var writeScroll = function (value) {
      viewport.scrollLeft = value;
      lastWritten = viewport.scrollLeft;
    };
    var sizeRail = function () {
      rail.style.setProperty('--rail-width', viewport.clientWidth + 'px');
      var offset = span ? ((viewport.scrollLeft % span) + span) % span : 0;
      span = group.getBoundingClientRect().width;
      position = motionPreference.matches ? 0 : span + Math.min(offset, span);
      writeScroll(position);
    };
    sizeRail();
    if ('ResizeObserver' in window) new ResizeObserver(sizeRail).observe(viewport);
    else window.addEventListener('resize', sizeRail);
    if (document.fonts) document.fonts.ready.then(sizeRail);
    motionPreference.addEventListener('change', sizeRail);
    ...
    viewport.addEventListener('pointerdown', function (e) {
      ...
      touching = true;
      resumeAt = Infinity;
      if (e.pointerType === 'mouse' && !motionPreference.matches) {
        dragging = true;
        dragX = e.clientX;
    ...
    viewport.addEventListener('pointermove', function (e) {
      if (dragging) viewport.scrollLeft = dragScroll + dragX - e.clientX;
    });
    ...
    viewport.addEventListener('lostpointercapture', endInteraction);
    // Browsers cancel pointer events when native touch scrolling takes over.
    // Keep the rail paused until the actual finger leaves, including a still hold.
    viewport.addEventListener('touchstart', function () { nativeTouchActive = true; }, { passive: true });
    var endTouch = function (e) {
      if (!nativeTouchActive || e.touches.length) return;
      nativeTouchActive = false;
      resumeAt = performance.now() + 1500;
    };
    window.addEventListener('touchend', endTouch, { passive: true });
    window.addEventListener('touchcancel', endTouch, { passive: true });
    viewport.addEventListener('wheel', function () { resumeAt = performance.now() + 1500; }, { passive: true });
    // Native touch, trackpad and keyboard scrolling keep extending the idle delay.
    ...
    if ('IntersectionObserver' in window) {
      new IntersectionObserver(function (entries) { inView = entries[0].isIntersecting; }).observe(rail);
    }
    var moveRail = function (now) {
      var elapsed = lastFrame ? Math.min(now - lastFrame, 64) : 0;
      lastFrame = now;
      var paused = motionPreference.matches || hovering || touching || nativeTouchActive || !inView ||
        document.hidden || viewport.matches(':focus-visible') || now < resumeAt;
      if (paused) position = viewport.scrollLeft;
      else if (span > 0) {
        position += elapsed * 0.016;
        // Rebase to the identical middle copy only while automatic motion runs.
        position = span + ((position - span) % span + span) % span;
        writeScroll(position);
      }
