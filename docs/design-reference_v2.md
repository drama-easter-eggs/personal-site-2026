...
> Warm storybook on cream paper — a friendly editorial canvas where oversized Inter headlines and paper-cut characters share a soft, sticker-like surface, anchored by a single teal accent.

**Theme:** light

MindMarket is a warm, illustrated editorial system built on a cream-paper canvas rather than stark white, with massive Inter display type that fills the frame and a single teal accent that anchors the brand across navigation strokes, borders, buttons, and the footer band. The visual language borrows from paper-cut storybook illustration — flat, vibrant character art sits directly on warm neutral backgrounds, never on photographic or gradient surfaces, and the UI chrome is deliberately minimal so the artwork leads. Components are generously rounded (50–64px radii on cards and nav), creating a soft, sticker-like quality. Color is deliberately monochromatic: one teal hue in three steps (Deep, Primary, Tint) carries every accent role. Primary Teal is reserved for buttons, borders, and small color blocks; Deep Teal takes the large fills (footer band, service CTAs); Teal Tint is the only chromatic surface. There is no second accent and no semantic state palette. The overall density is breathable and confident — few elements per screen, enormous type, wide margins, and the cream canvas doing the structural work that shadow systems usually handle in product UIs.

## Tokens — Colors

| Name | Value | Token | Role |
|------|-------|-------|------|
| Primary Teal | `#5cb2a0` | `--color-primary-teal` | The single brand accent — navigation strokes, card borders, menu toggle fill, CTA icon dots, tag outlines, small color blocks. Never used for large surface fills or body text |
| Deep Teal | `#2e9a82` | `--color-deep-teal` | Large-area accent — footer band and service-card CTA fills. White text on top. The only place teal appears as a big surface |
| Teal Tint | `#dcefe9` | `--color-teal-tint` | Soft chromatic surface — filled tag chips, long-read content blocks, subtle highlights. Quiet enough to read several paragraphs on |
| Cream Paper | `#f6f2e8` | `--color-cream-paper` | Dominant page background and soft card surface. Warm off-white with slightly less yellow than before, so teal sits cleanly on it |
| Sandstone | `#e3dcd0` | `--color-sandstone` | Secondary surface tone for inset or recessed card states and section backgrounds. Slightly deeper and a touch rosier than the cream canvas |
...
| Pure White | `#ffffff` | `--color-pure-white` | Elevated card surfaces, floating nav background, text on Deep Teal. The highest surface level in the stack |
...
| Pure Ink | `#000000` | `--color-pure-ink` | Icon fills and high-contrast borders only. Never for body text on cream |

**Retired:** Fresh Grass `#8ed462`, Sky Pop `#2ba0ff`, Coral Pop `#ff705d`, Sunshine Pop `#f5e211`. Their UI roles all map to the three teal steps above. Illustrations may still carry their own flat fills, but UI chrome uses teal only.
...
White or Sandstone card with a #2e9a82 Deep Teal filled button (pill shape, 50px radius, white text, small white circle icon at right) as the action trigger. Deep Teal is the most prominent chromatic action surface on the site and is reserved for service-level CTAs rather than global actions. A card carries either a Primary Teal border or a Deep Teal CTA, not both.
...
#2e9a82 Deep Teal fill section at the page bottom with white (#ffffff) text. A solid, calm closing band that contrasts with the cream canvas above.

### Long-read Content Block
**Role:** Multi-paragraph reading surface

#dcefe9 Teal Tint surface with 50px radius and 20px padding, body text in #2c2e2a. The only chromatic surface allowed under long text.
...
- Use #dcefe9 Teal Tint for filled chips and long-read blocks.
...
- Don't reintroduce a second accent hue (blue, coral, yellow). Hierarchy comes from the three teal steps, not from additional colors.
- Don't use teal as a functional state for success/error/warning.
...
- Don't use more than one teal step per UI component — a button is either Deep Teal filled, or light-ghost with a single Primary Teal dot; a card has either a Primary Teal border or a Deep Teal CTA, never both.
...
| 3 | Teal Tint | `#dcefe9` | Chromatic surface for chips and long-read blocks |
...
- chromatic surface: #dcefe9 (Teal Tint)
...
4. *Create a service action card:* Sandstone (#e3dcd0) or white card (50px radius, 21px padding) containing a Deep Teal (#2e9a82) pill button (50px radius, 11px vertical and 20px horizontal padding, 15px Inter 500 white text, small white circle icon at right). No teal border on this card.

5. *Create a footer accent band:* Full-width Deep Teal (#2e9a82) section at the page bottom, 50px+ padding, white (#ffffff) text, serving as a calm closing visual.
