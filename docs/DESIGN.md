...
| 角色 | 字級 | 字重 | 行高 | 字距 |
|------|------|------|------|------|
| hero 問句（兩行同級）`.hero__fixed` / `.hero__rotator` | 21 → 48px | 900 | 1.45 | .01em |
...
- **兩行同級：** Mei 指定兩行一樣粗、一樣大。字級上限因此由**最長那句**決定
  （14 個字 = 14.14em，必須在 col 1 裡停成一行）。最緊的不是桌機，是 **901px**——
  兩欄還沒併成一欄、右欄卻已經吃掉 330px，col 1 只剩 445px，換算 vw 上限 3.49，
  所以 `--fs-hero` 取 `clamp(21px, 3.4vw, 48px)`。**調大 vw 之前先量 901px。**
