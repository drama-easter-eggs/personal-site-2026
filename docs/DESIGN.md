...
桌面首頁介紹欄由 330px 放寬到 390px；541–900px 插圖與介紹並排，較窄手機使用縮小的插圖與單欄介紹。
...
在 index.html 裡排在 `</h1>` 之後。

- **放在哪：** 位置跟前一版的貓一樣——hero grid 第 2 列的左欄。那一格本來就是空的
  （問句吃掉整幅寬 `grid-column: 1 / -1`，右欄的自我介紹掉到第二列），插圖是填進
  這塊留白：`align-items: end` 讓兩欄齊底，1200px 下列高仍由右欄那三段文字決定
  （插圖 211px、文字 216px），**桌機 Hero 的總高度沒有變**（實測 659 → 663px）
...
- **裁切與改色（`reference/hero.svg` → `assets/img/hero-figure.svg`）：** 三件事，
  ...
  1. 原檔畫布 810×810，墨跡只佔 `13,126` 起算的 784×558（逐像素量出來的）。
     `viewBox` 改成 `13 126 784 558`，版面上的盒子才等於插圖本身，能真的貼齊左欄的
     文字邊；同時拿掉固定的 `width/height="1080"`，才吃得到 CSS 的寬度
...
- **尺寸：** `clamp(208px, 21vw, 296px)`，比例 784/558 ≈ 1.4 的橫幅，所以尺寸是倒過來
  從高度決定的——21vw 時高度落在 210px 上下，剛好跟右欄那三行自我介紹同一階。
  `margin-bottom: .6em` 補掉右欄最後一行字下方的 padding 與行距，插圖底部才跟那行字
  收在同一條線
- **方向：** 人物本來就朝右（往版面內側跳），不需要像貓那樣 `scaleX(-1)` 鏡像
- **541–900px：** 維持並排（不要拆成上下兩列——插圖自己佔一列會把 Hero 撐高 250px），
  左欄改成 `minmax(0, 32%)`、插圖 `width: 100%`，右欄至少還留 2/3 給文字。
  834px 實測 476 → 504px
- **≤540px：** 單欄直排，插圖 `min(232px, 62vw)` 夾在問句與自我介紹之間（543 → 614px）
- **進場：** 吃 hero 那組 `rise`，`animation-delay: .42s` 排在自我介紹（.3s）之後——
  先讀完字，插圖才出現。`prefers-reduced-motion` 由全站那條總開關關掉，
  這一版不需要任何 JS（lottie 引擎與 `loader-cat.js` 都已刪除）
