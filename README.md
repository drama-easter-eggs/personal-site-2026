...
`height: 340svh` 的跑道加一格 `position: sticky` 的舞台，捲動位置由 `main.js`
換算成 `--p`（0 → 1）寫在那一段上，所有位移與透明度都由 CSS 從 `--p` 推出來。
領域收回圓心、中央宣言與人物出現後，十個人性需求標籤會左右交錯向外彈出，帶輕微回彈；反向捲動會收回，末段保留閱讀時間。標籤沿用內文字體，配色與位置參考需求示意圖。
窄／矮視窗把標籤排成正常流；沒有 JS、或開啟「減少動態效果」時不 pin，退回一份靜態、完整的索引與需求標籤
（詳見 `docs/DESIGN.md`）。
