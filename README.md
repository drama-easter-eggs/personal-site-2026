...
`height: 500svh` 的跑道加一格 `position: sticky` 的舞台，捲動位置由 `main.js`
換算成 `--p`（0 → 1）寫在那一段上，所有位移與透明度都由 CSS 從 `--p` 推出來。
領域收回圓心、中央宣言與人物出現後，八個需求 bubble 依序浮現，停留閱讀後，同一組 bubble 原位淡出需求、淡入對應體驗策略。中央螢光筆由「看見真實的需求」移到「轉化為未來體驗策略」。反向捲動會還原，兩階段各保留閱讀時間。色紙、圓形與人物延續原構圖。
窄／矮視窗把每組「需求 → 策略」排成正常流；沒有 JS、或開啟「減少動態效果」時不 pin，退回一份靜態、完整的索引與需求／策略對照
（詳見 `docs/DESIGN.md`）。
