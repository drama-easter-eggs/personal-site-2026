...
**首頁有一處會把畫面停住（pin）的場。** 「研究走過的領域 → 精選案例」使用
`height: 300svh` 跑道與 `position: sticky` 舞台，捲動位置由 `main.js` 換算為 `--p`。
前段維持五個研究領域的環形收斂；之後轉為左上結語、右上小幅人物插圖、下方八組需求與策略對照。
對照表使用完整版寬，桌面兩欄四列、手機單欄，以細線分列，不使用有機色塊。
需求與策略同時可讀，不再原位換面。窄／矮視窗、無 JS 或「減少動態效果」時，
使用正常流排版；結語、插圖與對照表仍共同組成完整閱讀區塊。
